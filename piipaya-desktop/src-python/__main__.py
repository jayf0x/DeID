...
from engine import EntityHandler, MVP_ENTITY_TYPES, CoreEngine, CoreMapper
...
# currently no support for preview, but we do support conversion
...
class SidecarService:
    def __init__(self, root_dir: Path) -> None:
        ...
        self.mapper: CoreMapper | None = None
    ...
    def _plugin_dir(self) -> Path:
        raw = str(
            self.bootstrap.get(
                "plugin_dir",
                "~/Library/Application Support/piipaya/plugins/",
            )
        )
    ...
    def _load_plugin_handlers(self) -> dict[str, EntityHandler]:
        ...
        for plugin_file in sorted(plugin_dir.glob("*.py")):
            module_name = f"piipaya_plugin_{plugin_file.stem}"
            try:
