<script lang="ts">
  ...
  const PROFILE_STORAGE_KEY = "piipaya:settings-profiles:v1";
  const ACTIVE_PROFILE_STORAGE_KEY = "piipaya:active-profile:v1";

  const entityFilters = entityFiltersCatalog();

  // DEV ONLY
  const defaultInput = `Look, I’m not saying I’m a hero. I’m just a guy who’s very comfortable in his own skin and deeply concerned about yours. Last Tuesday, April 07, 2026, I was lounging at 1428 Elm Street, Pasadena—great light in the afternoons, really highlights my calves—when I saw my neighbor, Derrick. Not me, Derek. The other Derrick. The one with the bad vibes and the even worse digital footprint.
...
That’s why you need a Derek. Or a tool that acts like one. Someone to step in and say, "Let's turn 'Sarah Jenkins' into 'Stacy Miller' and 'General Hospital of Southern California' into 'City Wellness Center'." You keep the story, you keep the drama, but you lose the identity theft. Now, if you’ll excuse me, I have a self-care appointment at 4:00 PM. Privacy is the ultimate flex, Derrick. Get with the program.`;
  ...
  async function exportZipPlaceholder() {
    ...
    try {
      ...
      await saveBlobWithPicker(blob, "piipaya-export.zip");
  ...
  async function exportCurrentConfig() {
    try {
      ...
      await saveBlobWithPicker(blob, "piipaya-config.json");
